...
(defun compile-haskell ()
  (when (derived-mode-p 'sr-mode 'haskell-mode)
	...
	(when (string-match "Diagram B" (buffer-string))
...
** Google Translate
:PROPERTIES:
:ID:       239dabe3-fe1f-498c-93e7-739b8ebdce1a
:END:
#+begin_src emacs-lisp
(require 'google-translate)
(require 'google-translate-default-ui)

(setq google-translate-default-source-language "iw")
(setq google-translate-default-target-language "en")

(defun translate-hebrew (text)
  (let* ((translation-newline (google-translate-translate "iw" "en" text 'echo-area))
         (translation-raw (replace-regexp-in-string "\n" " " translation-newline)))
    (save-match-data
      (replace-regexp-in-string "Hebrew -> English: .*? - " "" translation-raw))))
#+end_src
** Latex
:PROPERTIES:
...
(setq org-latex-pdf-process (quote ("texi2dvi -p -b -V --build-dir %o -o %o%b.pdf %f")))
;; (setq org-latex-pdf-process (quote ("pdflatex --shell-escape -output-directory %o %f")))
